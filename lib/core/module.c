...
module_load_symbols(const module_entry_t* entries, const lisp_t lisp,
                    const atom_t symbols, const atom_t result)
{
  TRACE_MODL_SEXP(symbols);
  /*
   ...
   * Grab CAR and CDR.
   */
  atom_t car = lisp_car(lisp, symbols);
  atom_t cdr = lisp_cdr(lisp, symbols);
  X(lisp->slab, symbols);
  /*
   * Process the remainder.
   */
  atom_t nxt = module_load_symbols(entries, lisp, cdr, result);
  /*
...
static atom_t
module_load_entries(const module_entry_t* entries, const lisp_t lisp,
                    const atom_t symbols, const atom_t result)
{
  atom_t nxt = result;
  /*
   ...
   */
  return module_load_symbols(entries, lisp, symbols, result);
}

atom_t
module_load_shared(const char* const path, const lisp_t lisp, const atom_t name,
                   const atom_t symbols)
{
  ...
  const atom_t mods = module_load_entries(e, lisp, symbols, scope);
  if (IS_NULL(mods)) {
    DLCLOSE(add_to_cache, handle);
    return mods;
  }
...
module_load(const lisp_t lisp, const atom_t cell)
{
   ...
   */
  atom_t syms = module_load_shared(path, lisp, module_name, symbol_list);
  /*
   * Clean-up and return.
   */
